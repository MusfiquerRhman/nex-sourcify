...
import type { factoryInvoiceRouter } from '~/server/api/routers/commercial/factoryInvoice'; 
import { currencyFormatter } from "~/utils/localNumberStrings";
import { useDecodedUser } from "~/hooks";

type InvoiceOutput = inferRouterOutputs<typeof factoryInvoiceRouter>;

type GetPDFDataOutput = InvoiceOutput['getPDFData'];

registerPdfFonts();

const FactoryInvoicePDF = ({ data }: { data: GetPDFDataOutput }) => {
	const tableColumns = [
		{ key: 'brand', label: 'Brand', width: 250},
		{ key: 'buyer_po', label: 'Order/PO No', width: 250},
		{ key: 'style', label: 'Style No', width: 250},
		{ key: 'delivery_quantity', label: 'Quantity', width: 110 },
    ...
    return (
		<Document
            title={`Factory Invoice - ${data.header?.invoice_no}`}
            author="Nex Sourcify"
        ...
        >
			<Page orientation="portrait" size="A4" style={styles.body} wrap>
				<NexSourcifyLogo />
				<ReportTitle title={data.header?.factory_name!} />
				<View style={{
					display: "flex",
					alignItems: "center",
					justifyContent: "center",
				}}>
					<Text style={styles.subtitle}>{`${data.header?.factory_address}`}</Text>
				...
				<View style={styles.doubleColumn}>
					...
					<View style={{flex: 1, gap: 6}}>
						...
						<View style={styles.flex_c_5}>
							<Text style={styles.textBold}>{`Exporter Name and Address:`}</Text>
							<View style={styles.border}>
								<Text style={{...styles.textAmasis, marginBottom: 2}}>{data.header?.factory_name ?? ''}</Text>
								<Text style={{...styles.textAmasis, lineHeight: 1.2}}>{data.header?.factory_address ?? ''}</Text>
							</View>
						</View>
						<View style={styles.flex_c_5}>
						...
						<View style={{...styles.flex_c_5, gap: 3}}>
							<Text style={styles.textBold}>{`Exporter Bank Details:`}</Text>
							<Text style={{...styles.textAmasis, lineHeight: 1.2}}>
								{data.header?.factory_bank_name}
							</Text>
							<Text style={{...styles.textAmasis, lineHeight: 1.2}}>
								{data.header?.factory_bank_address}
							</Text>
							<View style={{display: 'flex', flexDirection: 'row', justifyContent: 'space-between'}}>
								...
								<View style={{...styles.text, display: 'flex', flexDirection: 'column', flex: 1}}>
									<Text style={{...styles.textAmasis, marginBottom: 3}}>{data.header?.factory_bank_account_name ?? ''}</Text>
									<Text style={{...styles.textAmasis, marginBottom: 3}}>{data.header?.factory_bank_swift_code?.trim() ?? ''}</Text>
									<Text style={{...styles.textAmasis, marginBottom: 3}}>{data.header?.factory_bank_account_no ?? ''}</Text>
								</View>
							</View>
						</View>
					</View>
							...
							<View>
								...
								{data.header?.consignees?.map((consignee, index) => (
									<View key={index} style={{marginTop: 5, display: 'flex', flexDirection: 'column', gap: 2, marginBottom: 10}}>
										...
										<Text style={styles.textAmasis}>{consignee.buyer_consignee?.consignee_name}</Text>
										<Text style={styles.textAmasis}>{consignee.buyer_consignee?.address}</Text>
									</View>
								))}
							</View>
						}
					...
					<View style={{flex: 1, gap: 6}}>
						{data.header?.notify_parties && data.header?.notify_parties.length > 0 &&
							<View>
								...
								{data.header?.notify_parties?.map((notifyParty, index) => (
									<View key={index} style={{marginTop: 5, display: 'flex', flexDirection: 'column', gap: 2, marginBottom: 10}}>
										<Text style={{
											fontFamily: 'Amasis',
											fontSize: 10,
										...
										<Text style={styles.textAmasis}>{notifyParty.buyer_consignee?.consignee_name}</Text>
										<Text style={styles.textAmasis}>{notifyParty.buyer_consignee?.address}</Text>
									</View>
								))}
							</View>
						}
