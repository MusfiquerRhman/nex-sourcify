...
const TNATemplates = () => {
    ...
    const {data: tnaTemplates, isLoading} = api.tnaTemplates.getTnaTemplates.useQuery({
        limit,
        offset: page * limit,
    });
    ...
    const searchQuery = api.tnaTemplates.searchTnaTemplates.useQuery(
        { query: debouncedSearch, limit, offset: page * limit },
        { enabled: debouncedSearch.length > 0 }
    );

    const editURL = '/merchandising/tna/tna_templates/edit/';

    const deleteMutation = api.tnaTemplates.deleteTnaTemplate.useMutation({
        onSuccess: async () => {
            toast.success("TNA Template deleted successfully!");
            await Promise.all([
                utils.tnaTemplates.getTnaTemplates.invalidate(),
                utils.tnaTemplates.searchTnaTemplates.invalidate()
            ]);
        },
    });
                    ...
                    can_add ? (
                        <div className="w-70">
                            <Button
                                ...
                                leftIcon={plusIcon}
                                onClick={() => router.push('/merchandising/tna/tna_templates/new')}
                            />
                        </div>
                    ) : null
                }
