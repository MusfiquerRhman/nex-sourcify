...
import type { GetFreightTermByIdTypes } from "~/types/libraryAPITypes";

export const useFreightTerm = (freightTermData?: GetFreightTermByIdTypes) => {
    // Form setup
    const methods = useForm<FormValues>({
        resolver: zodResolver(formSchema),
        defaultValues: freightTermData ?? {}
    });
    ...
    // Effect to reset form when freightTermData changes
    useEffect(() => {
        if (freightTermData) {
            methods.reset({
                ...freightTermData,
