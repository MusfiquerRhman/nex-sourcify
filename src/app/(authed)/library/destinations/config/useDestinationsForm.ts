...
import type { GetDestinationByIdTypes } from "~/types/libraryAPITypes";

export const useDestinationsForm = (destinationData?: GetDestinationByIdTypes) => {
    // Form setup
    const methods = useForm<FormValues>({
        resolver: zodResolver(formSchema),
        defaultValues: destinationData ? {
                name: destinationData.name ?? undefined,
                country_id: destinationData.country_id?.toString(),
              }
            : {},
    });
    ...
    // Effect to reset form when destinationData changes
    useEffect(() => {
        if (destinationData) {
            const transformedData = Object.fromEntries(
                Object.entries({
                    name: destinationData.name,
                    country_id: destinationData.country_id ? destinationData.country_id.toString() : undefined,
                }).map(([key, value]) => [key, value === null ? undefined : value])
            );
            methods.reset(transformedData);
        }
    }, [destinationData, methods]);
