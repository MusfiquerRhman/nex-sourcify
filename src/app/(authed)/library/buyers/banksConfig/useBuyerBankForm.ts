...
const banksFormSchema = z.object({
    banks: z.array(formSchema),
});

export const useBanksForm = (bankData?: GetBuyerByIdTypes['banks']) => {
    // Form setup
    const { reset, register, control, handleSubmit, formState: { errors: validationError }, trigger } = useForm<BanksFormValues>({
        resolver: zodResolver(banksFormSchema),
        defaultValues: {
            banks: [],
        },
    });
